...
$fullPage = ($pageNum == 1 || $pageNum == 2 || $pageNum == 3 || $pageNum == 4) ? true : false;
$subPage = ($pageNum==5 || $pageNum==100) ? true : false;
?>

<style>
.logoArea { width: 300px; height: 100%; border-right: 1px solid rgba(255,255,255,0.5); }
.logo { display: block; padding: 35px; cursor: pointer; }

.callArea { width: 197px; height: 100px; background: #333333; position: absolute; right: 0; top: 0; text-align: center; color: #fff; font-size: 16px; font-weight: bold; }
.callArea > img { padding: 42px 10px 0 0; vertical-align: top; }
.callArea > span { display: inline-block; padding: 28px 0; text-align: left; }

#Menu { position: absolute; top: 0px; right: 230px; }
#Menu > li { float: left; text-align: center;  margin-left: 30px; }
#Menu > li:first-child { margin-left: 0px; }
#Menu > li > a { display: block; position: relative; color: #fff; font-size: 18px; font-weight: 500; line-height: 100px; padding: 0 15px; }
#Menu > li > a > div { width:0; height:2px; background:#fff; position:absolute; bottom:-1px; left:0; transition:all .3s ease-out; }
#Menu > li.on > a > div,
#Menu > li > a:hover > div { width:100%; }
</style>

<div class="wrap">
<div class="wrap-header">
	<header class="layout" <?=$pageNum==2 ? "style='background: rgba(0,0,0,0.5);'" : ""?> >
		<div class="logoArea"><img src="/res/images/top/logo.png" alt="로고" onclick="home()" class="logo" /></div>
		
		<div class="callArea">
			<img src="/res/images/top/call.png" alt="전화번호" />
			<span>
				064.792.3112<br>
				010.7124.3111
			</span>
...
<?if($subPage) { ?>
	<div class="subvisual" style="background-image:url('/res/images/subvisual/<?=$Svisual?>.jpg');" >
		...
		<?}
				...
				if($subNum == 7) echo "<p>PRIVACY</p><p>개인정보처리방침</p>";
				else echo "<p>LOGIN</p><p>관리자 페이지</p>";
			}
		?>
	</div>
<?}?>
...
<script>
	...
	$(function(){
		...
		$(".resize > div").css("height", $(window).height());
	});
	...
	function check_scroll(){
        ...
        if(scrollHeight - footerHeight < scrollTop + $(window).height())
            $(".quick_menu").css({"position":"absolute", "bottom":(footerHeight+50)+"px"});
        else
            $(".quick_menu").css({"position":"fixed", "bottom":"80px"});

		if (scrollTop > 100  && scrollact == false) {
			$(".quick_top").show();
			$(".quick_top").stop().animate({"left" : "0px", "opacity" : "1"});
			
			$(".quick_chat").stop().animate({ "top":"0px" },"easeOutBack");
			
			scrollact = true;
		} 
		
		if (scrollTop < 100) {
			$(".quick_top").stop().animate({"left" : "45px", "opacity" : "0"},function(){
				$(".quick_top").hide();
			});
			$(".quick_chat").stop().animate({ "top":"76px" },"easeOutBack");
			scrollact = false;
		}
	}
</script>